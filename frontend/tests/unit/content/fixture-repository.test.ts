...
describe("articles", () => {
  it("publishes the two zh-CN fixture guides", async () => {
    ...
    for (const article of articles) {
      ...
      expect(article.category.slug).toBe("guides");
      expect(article.category.label).toBe("指南");
  ...
  it("exposes categories and filters by them", async () => {
    const categories = await repo.listArticleCategories(LOCALE);
    expect(categories.map((c) => c.slug)).toEqual(["guides"]);
