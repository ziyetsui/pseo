...
import { ModelPromptExplorer } from "./ModelPromptExplorer";
 ...
 *
 * Everything here is a server component. `ModelPromptExplorer` is the one
 * client leaf, and it receives these bands as slots, so the exported HTML
 * carries the full listing whether or not JavaScript runs. Each grid uses its
 * own `idPrefix` because the same prompt can appear in two bands — two copies
 * must not collide on one `<pre>` id.
 */
export function ModelBrowse({
  ...
  return (
    <ModelPromptExplorer
      ...
      searchPlaceholder={`描述你想要的画面，或搜索下方 ${prompts.length} 条提示词…例如：杂志感美妆人像、奢侈品静物`}
      hero={<ModelIdentity model={model} />}
      generateControls={<ModelGenerateControls />}
      beforeAllPrompts={
