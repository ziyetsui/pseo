 ...
 * server-rendered page and the client `VariableSelector` can render it with the
 * text each of them owns. Hidden from `md` up, where the inline copy button is
 * already on screen; the page reserves matching bottom padding so nothing is
 * ever covered.
 */
export function StickyCopyBar({
  ...
  return (
    <aside
      ...
      className="fixed inset-x-0 bottom-0 z-40 border-t-2 border-foreground bg-surface md:hidden"
    >
