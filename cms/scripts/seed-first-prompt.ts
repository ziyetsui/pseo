...
import { runFirstPromptSeedCli } from '../src/seed/firstPromptCli.ts'
import type { PayloadContentValidationApi } from '../src/publication/payloadDraftContentValidator.ts'
import type { FirstPromptSeedPayloadApi } from '../src/seed/firstPrompt.ts'
...
async function main(): Promise<void> {
  ...
  await runFirstPromptSeedCli({
    argv: process.argv.slice(2),
    cwd: process.cwd(),
    loadEnv: (cwd) => loadEnvConfig(cwd, process.env.NODE_ENV !== 'production'),
    loadConfig: async () => (await import('../src/payload.config.ts')).default,
    getPayload: async ({ config }) => await getPayload({ config } as never) as unknown as FirstPromptSeedPayloadApi & PayloadContentValidationApi,
...
void main().catch((error: unknown) => {
  ...
  process.stderr.write(`first Prompt seed failed: ${message}\n`)
  process.exitCode = 1
})
